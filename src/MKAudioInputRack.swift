...
final class MKAudioInputRack: NSObject {
    ...
    private final class StageHost {
        ...
        init(audioUnit: AVAudioUnit,
            ...
            let selectedFormats = try StageHost.configureFormats(for: audioUnit.auAudioUnit,
        ...
        private static func zero(buffer: AVAudioPCMBuffer, frameCount: Int) {
            ...
            for audioBuffer in bufferList {
                ...
                let byteCount = buffer.format.isInterleaved
                    ? frameCount * Int(buffer.format.channelCount) * bytesPerChannelFrame
                    : frameCount * bytesPerChannelFrame
    ...
    private final class VST3StageHost {
        ...
        func process(samples: UnsafeMutablePointer<Float>, frameCount: Int, hostChannels: Int) {
            ...
            do {
                ...
                print("MKAudioRack: Input Track stage render failed \(pluginHost.displayName): \(error.localizedDescription)")
                return
            }
    ...
    func updateAudioUnitChain(_ stages: NSArray?, sampleRate: UInt) {
        ...
        stateLock.unlock()
        shutdownStageHosts(oldHosts)
        let newHosts = buildStageHosts(from: normalized,
                                       sampleRate: rebuildSampleRate,
                                       hostBufferFrames: rebuildBufferFrames)
        stateLock.lock()
        stageHosts = newHosts
    ...
    func processSamples(_ samples: UnsafeMutablePointer<Int16>, frameCount: UInt, channels: UInt, sampleRate: UInt) {
        ...
        if !hosts.isEmpty {
            ...
            while offset < localFrameCount {
                ...
                let chunkPointer = floatBuffer.advanced(by: offset * localChannels)
                for host in hosts {
        ...
        outputPeak = computePeak(floatBuffer, sampleCount: sampleCount)
        for index in 0..<sampleCount {
            let scaled = max(-32768.0, min(32767.0, floatBuffer[index] * 32767.0))
    ...
    private func normalizeStages(_ stages: NSArray?) -> [StageConfiguration] {
        ...
        for element in stages {
            ...
            if let audioUnit = dictionary["audioUnit"] as? AVAudioUnit {
                ...
                normalized.append(StageConfiguration(processor: .vst3(vst3Host), wetDryMix: min(max(wetDryMix, 0.0), 1.0), sidechainSourceKey: sidechainSource))
            }
        }
        return normalized
    }
